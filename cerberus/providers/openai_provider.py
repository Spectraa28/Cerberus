...
from cerberus.providers.base import Turn, UserTurn, AssistantTurn, ToolResultTurn, ToolCall, NormalizedResponse, Usage
...
class OpenAIProvider:
    ...
    def __init__(self, model: str, max_tokens: int = 1024) -> None:
        ...
        self.client = OpenAI(api_key=os.environ[self.api_key_env], base_url=self.base_url)
    ...
    async def call(self, history: list[Turn], tool_schemas: list[dict]) -> NormalizedResponse:
        ...
        return NormalizedResponse(text=msg.content, tool_calls=tool_calls, stop_reason=stop_reason, usage=usage)